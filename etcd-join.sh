...
if [[ ${#REMOTE_SSH_KEY} == 0 ]]
then
    export $(ssh -o StrictHostKeyChecking=no -l ${REMOTE_SSH_USER} $REMOTE_SSH_IP docker exec etcd ${ETCD_ADD_MEMBER_CMD} | grep ETCD_INITIAL_CLUSTER)
    else
    export $(ssh -o StrictHostKeyChecking=no -i ${REMOTE_SSH_KEY} -l ${REMOTE_SSH_USER} $REMOTE_SSH_IP docker exec etcd ${ETCD_ADD_MEMBER_CMD} | grep ETCD_INITIAL_CLUSTER)
fi
...
echo ${green}Launching clean etcd in order to rejoin cluster${reset}
...
if [[ ${#REMOTE_SSH_KEY} == 0 ]]
then
    ssh -o StrictHostKeyChecking=no -l ${REMOTE_SSH_USER} $REMOTE_SSH_IP docker exec etcd etcdctl member list
    else
        ssh -o StrictHostKeyChecking=no -i ${REMOTE_SSH_KEY} -l ${REMOTE_SSH_USER} $REMOTE_SSH_IP docker exec etcd etcdctl member list
fi
